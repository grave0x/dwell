...
impl SecretBackend for AgeBackend {
    ...
    fn encrypt(&self, _key: &str, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        let key_file =
            std::fs::read_to_string(&self.identity_path).map_err(dwell_core::DwellError::Io)?;
        ...
        let public_key = key_file
            ...
            .map(|s| s.trim())
            .ok_or_else(|| {
                dwell_core::DwellError::Encryption(
        ...
        let mut child = std::process::Command::new(tool)
            ...
            .spawn()
            .map_err(|e| {
                dwell_core::DwellError::Encryption(format!("Failed to spawn {}: {}", tool, e))
            })?;
        ...
        if output.status.success() {
            ...
            Err(dwell_core::DwellError::Encryption(format!(
                "age encrypt failed: {}",
