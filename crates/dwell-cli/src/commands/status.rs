...
pub fn run(cli: &CliRef, _cfg: &dwell_core::Config, source: Option<PathBuf>) -> dwell_core::Result<()> {
    let _ = _cfg;
    ...
    out.success(&format!("{} in sync, {} modified/missing, {} total",
        in_sync, modified + missing, entries.len()));
