...
pub fn run(cli: &CliRef, _cfg: &dwell_core::Config, source: Option<PathBuf>, force: bool) -> dwell_core::Result<()> {
    let _ = _cfg;
    ...
    out.success(&format!("{} entries applied ({} skipped)", applied,
        results.len() - applied));
...
fn load_template_data(home: &PathBuf) -> serde_json::Value {
    ...
    data.insert("os".into(), serde_json::Value::String(
        std::env::consts::OS.to_string()
    ));
    data.insert("arch".into(), serde_json::Value::String(
    ...
    data.insert("platform".into(), serde_json::Value::String(platform.to_string()));
