...
/// Built-in module: programs.shell — minimal shell defaults.
...
impl Module for ShellModule {
    ...
    fn description(&self) -> &str {
        "Configure default shell settings and prompt tooling"
    }
    ...
    fn options(&self) -> Vec<ModuleOption> {
        vec![
            ModuleOption {
                name: "default".into(),
                option_type: OptionType::Enum(vec!["bash".into(), "zsh".into(), "fish".into()]),
                default: Some(serde_json::json!("bash")),
                description: "Default shell".into(),
                example: Some("zsh".into()),
                required: false,
            },
            ModuleOption {
                name: "prompt".into(),
                option_type: OptionType::Enum(vec!["starship".into(), "oh-my-posh".into()]),
                default: None,
                description: "Prompt tool".into(),
                example: Some("starship".into()),
                required: false,
            },
            ModuleOption {
                name: "aliases".into(),
                option_type: OptionType::Dict(std::collections::HashMap::new()),
                default: None,
                description: "Shell aliases".into(),
                example: Some(r#"{"ll":"ls -la","cat":"bat"}"#.into()),
                required: false,
            },
        ]
    }

    fn generate(&self, _values: &serde_json::Value) -> Result<Vec<Entry>> {
        Ok(vec![Entry {
            source_path: "dot_shellrc".into(),
            kind: EntryKind::File,
            content_hash: dwell_core::hash_content(b"# generated by programs.shell\n"),
            encrypted: false,
        }])
    }
}

/// Built-in module: programs.ssh — basic SSH config options.
...
impl Module for SshModule {
    ...
    fn description(&self) -> &str {
        "Generate ~/.ssh/config from typed options"
    }
    ...
    fn options(&self) -> Vec<ModuleOption> {
        vec![
            ModuleOption {
                ...
                option_type: OptionType::String,
                default: None,
                description: "Default host alias".into(),
                example: Some("github.com".into()),
                required: false,
            },
            ModuleOption {
                name: "user".into(),
                option_type: OptionType::String,
                default: None,
                description: "Default SSH user".into(),
                example: Some("git".into()),
                required: false,
            },
            ModuleOption {
                name: "identity_file".into(),
                option_type: OptionType::Path,
                default: Some(serde_json::json!("~/.ssh/id_ed25519")),
                description: "Default private key path".into(),
                example: Some("~/.ssh/id_rsa".into()),
                required: false,
            },
            ModuleOption {
                name: "add_keys_to_agent".into(),
                option_type: OptionType::Boolean,
                default: Some(serde_json::json!(true)),
                description: "Add keys to SSH agent".into(),
                example: None,
                required: false,
            },
        ]
    ...
    fn generate(&self, _values: &serde_json::Value) -> Result<Vec<Entry>> {
        Ok(vec![Entry {
            source_path: "dot_ssh/config".into(),
            kind: EntryKind::File,
            content_hash: dwell_core::hash_content(b"# generated by programs.ssh\n"),
            encrypted: false,
        }])
    }
}
...
mod tests {
    ...
    #[test]
    fn test_shell_module_exists() {
        let module = ShellModule::new();
        assert_eq!(module.id(), "programs.shell");
        assert!(!module.options().is_empty());
    }

    #[test]
    fn test_ssh_module_exists() {
        let module = SshModule::new();
        assert_eq!(module.id(), "programs.ssh");
        assert!(!module.options().is_empty());
    }
}
